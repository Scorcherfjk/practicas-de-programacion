...
def res_long(datos):
	if datos == []:
		...
		print("Existencia |\t Nombre\t\t| Precio / Medida    |\t Total invertido")
		print("=================================================================================#")
		for i in datos:
...
def res_short(datos):
	if datos == []:
		...
		print("=========================================#")
		for i in datos:
			print("{codigo}.- {nombre} \t a S/ {precio} cada {medida} \t |".format(
				codigo=i[3], 
				nombre=i[0], 
				precio=i[1], 
				medida=i[2]
...
def prod_disp(cursor):
	...
	datos = cursor.execute("""
		SELECT productos.cantidad, productos.nombre, productos.precio, medidas.medida 
		FROM productos 
		INNER JOIN medidas 
...
def prod_poca_exis(cursor):
	...
	datos = cursor.execute("""
		...
		OR ((medidas.medida = 'kg' OR medidas.medida = 'lt') AND cantidad BETWEEN 0.01 AND 0.5)
		""").fetchall()
	res_long( datos )
...
def prod_exis(cursor):
	...
	datos = cursor.execute("""
		...
		INNER JOIN medidas 
		ON(medidas.id = productos.tipo_medida) 
		WHERE productos.cantidad = 0""").fetchall()
	res_short(datos)
...
def prod_total(cursor):
	...
	datos = cursor.execute("""
		...
		FROM productos 
		INNER JOIN medidas 
		ON(medidas.id = productos.tipo_medida)""").fetchall()
	res_short(datos)
